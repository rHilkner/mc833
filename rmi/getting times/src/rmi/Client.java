...
//            	input = sc.nextLine();
            	...
            	String[] inputs = {"all list",
            	                 "all info",
            	                 "MC102 title",
            	                 "MC102 menu",
            	                 "MC102 schedule",
            	                 "MC102 addCommentary abcdeabcde",
            	                 "MC102 commentary"};
            		...
            		for (int j = 0; j < 30; j++) {
            			...
            			if (discipline_id.equals("all")) {
                    		if (request_type.equals("list")) {
                    			server_interface.getAllDisciplinesList();
                    		...
                    		if (request_type.equals("title")) {
                    			server_interface.getDisciplineTitle(discipline_id);
                    		} else if (request_type.equals("menu")) {
                    			server_interface.getDisciplineMenu(discipline_id);
                    		} else if (request_type.equals("schedule")) {
                    			server_interface.getDisciplineSchedule(discipline_id);
                    		} else if (request_type.equals("commentary")) {
                    			server_interface.getDisciplineCommentary(discipline_id);
                    			...
                    			if (!user_permission.equals("professor")) {
                    				...
                    				commentary = inputs[i].substring(inputList[0].length() + inputList[1].length() + 2);
                    				server_interface.addCommentary(discipline_id, commentary);
                    			}
                    		} else {
                    			System.out.print(ERROR_MESSAGE);
                    		}
